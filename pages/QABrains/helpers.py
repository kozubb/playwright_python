...
class Helpers:
    ...
    def login_as(self, endpoint, username, password):
        ...
        login.fill_email(username)
        login.fill_password(password)
        login.press_login_button()

        # Verify the email is shown in the user name element
        expect(self.page.locator(".user-name")).to_have_text(username)

    # Remove currency symbols from a string and return numeric part
    def remove_currency_symbol(self, text):
        return re.sub(r"[\$£€+]", "", text).strip()

    # Remove numeric price and return only currency symbols
    def remove_price(self, text):
    ...
    def remove_letters_colon_and_space(self, text):
        return re.sub(r"[a-zA-Z:\s]", "", text)

    # Convert comma to dot and parse string to float
    def change_comma_sign(self, text):
    ...
    def price_validator(
        ...
        expected_price_as_number_fixed = round(expected_price_value, 2)
        ...
        currency_symbol = self.remove_special_chars(
            self.remove_price(currency_symbol_without_letters)
        )

        # Assert that numeric price matches expected
        assert round(price, 2) == expected_price_as_number_fixed
